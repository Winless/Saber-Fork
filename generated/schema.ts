...
export class UserClaimEntity extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id !== null, "Cannot save UserClaimEntity entity without an ID");
    assert(
      id.kind == ValueKind.STRING,
      "Cannot save UserClaimEntity entity with non-string ID. " +
        'Considering using .toHex() to convert the "id" to a string.'
    );
    store.set("UserClaimEntity", id.toString(), this);
  }

  static load(id: string): UserClaimEntity | null {
    return store.get("UserClaimEntity", id) as UserClaimEntity | null;
  }
